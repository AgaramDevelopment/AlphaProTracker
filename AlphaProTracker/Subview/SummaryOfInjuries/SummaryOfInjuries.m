...
@property (nonatomic,strong) IBOutlet NSMutableArray *injurylist;

@end

@implementation SummaryOfInjuries
    ...
    if([COMMON isInternetReachable])
    {
        ...
        [manager POST:URLString parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if(responseObject >0)
            {
                ...
                self.injurylist = [[NSMutableArray alloc]init];
                self.injurylist = [responseObject valueForKey:@"lstPlayerDashBoardInjury"];
                
            }
            
            
            
            NSLog(@"%@", self.injurylist);
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.date.text = [[self.injurylist valueForKey:@"onsetdate"] objectAtIndex:indexPath.row];
    cell.value.text = [[self.injurylist valueForKey:@"injuryname"] objectAtIndex:indexPath.row];
