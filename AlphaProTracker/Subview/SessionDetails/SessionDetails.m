    ...
    if([COMMON isInternetReachable])
    {
        ...
        [manager POST:URLString parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if(responseObject >0)
            {
                ...
                self.dailylist = [responseObject valueForKey:@"lstSessionDetailsDaily"];
                
                self.weeklylist = [[NSMutableArray alloc]init];
                self.weeklylist = [responseObject valueForKey:@"lstSessionDetailsWeek"];
                
                self.monthlylist = [[NSMutableArray alloc]init];
                self.monthlylist = [responseObject valueForKey:@"lstSessionDetailsMonth"];
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(isdaily == YES)
    {
        cell.date.text = [[self.arraylist valueForKey:@"SDDailyDate"] objectAtIndex:indexPath.row];
        cell.value.text = [[self.arraylist valueForKey:@"SDDailyBallCount"] objectAtIndex:indexPath.row];
    }
    
    if(isweekly == YES)
    {
        cell.date.text = [[self.arraylist valueForKey:@"SDWeek"] objectAtIndex:indexPath.row];
        cell.value.text = [[self.arraylist valueForKey:@"SDWeekCount"] objectAtIndex:indexPath.row];
    }
    if(ismonthly == YES)
    {
        cell.date.text = [[self.arraylist valueForKey:@"SDMonth"] objectAtIndex:indexPath.row];
        cell.value.text = [[self.arraylist valueForKey:@"SDMonthBallCount"] objectAtIndex:indexPath.row];
    }
